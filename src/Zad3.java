/*
Write a program that randomly selects three integers between <5, 10>.
The two largest numbers drawn are the number of rows and columns of a two-dimensional array of integers
(it doesn't matter which number will be the number of rows and which number will be the number of columns).
The table is filled with random values from the range <100, 999>. Then randomize the row index and the column index
table so generated. It is necessary to determine to which edge the sum of elements in a straight line from the indicated element should be determined
by the randomly selected indices is the largest, as shown in the example below.
We assume that the element marked in red has been determined.
For such an arrangement of elements, the largest sum is for the left edge because it is 7 + 6 = 13.
Other sums: 1 for the bottom edge, 2 for the right edge, and 2 + 8 = 10 for the top edge.
3 1 8 1
8 9 2 8
7 6 5 2
8 6 1 1
